    ...
    , ui(new Ui::MainWindow)
{
    ...
    this->initForm();
    /*
    ui->lineEdit->setText(QStringLiteral("C:/Users/MECHREVO/Desktop/材料出库明细表.xlsx"));		                               //测试写入路径
    */
}
...
void MainWindow::on_pushButton_jiexi_clicked()
{
    ...
    for (int i = 3; i < rowCounts; ++i)
    {
                ...
                && Wupin_Map[doc.read(i, 3).toString()] == doc.read(i, 6))                                //单据行不为空 且 单价相同
        {
            ...
            /*!
             *  doc.read(i, 5) 为 QVariant 类型
             *	读取数据为str
             *  转换为str后搜索字符串
             */
            
            /*! 判断数量是否为 double */
			QString name = doc.read(i, 5).toString();
            if (name.contains(".", Qt::CaseSensitive))
            {
                ui->textBrowser->append(QString("物品：%1 数量为小数。").arg(doc.read(i, 3).toString()));
            }
            ...
            /*单据当前物品信息不为空，则记录数量+当前读取数量*/
            else if (!Danju_data_Object[doc.read(i, 3).toString()].isNull())
            {
                Danju_data_Object.insert(doc.read(i, 3).toString(),
                                         doc.read(i, 5).toDouble() + Danju_data_Object[doc.read(i, 3).toString()].toDouble());//写入单据数据 物品名称 记录数量+读取数量
                Danju_Object.insert(doc.read(i, 1).toString(), Danju_data_Object);
            }
        }
        else if (doc.read(i, 1).toString() != ""
                 && Wupin_Map[doc.read(i, 3).toString()] != doc.read(i, 6)) /*!< 单据行不为空 且 价格不同 */
        {
            ...
            ui->textBrowser->append(QString("第 %1 行，%2 单价不同 记录值：%3 读取值：%4")
				.arg(i).arg(doc.read(i, 3).toString()).arg(Wupin_Map[doc.read(i, 3).toString()]).arg(doc.read(i, 6).toString()));
        }
        else if (doc.read(i, 1).toString() == "")
        {
    ...
    xlsxDoc.setColumnWidth(1, 13);/*!< 设置第一列宽13 */
    //xlsxDoc.setRowHeight(1，10);/*!< 设置第一行高10 */
    ...
    */
        ...
        for (int i = 2, k = 0; k < list_wupin.size(); ++k)
        {
            if (!json_Danju[xlsxDoc.read(1, i).toString()].isNull())
            {
                ...
                if (ui->checkBox->isChecked() == true)
                {
                    ...
                    Number = Number + json_Danju[xlsxDoc.read(1, i).toString()].toDouble();                              /*!< 加入数量 */
                }
                ...
                if (ui->checkBox_2->isChecked() == true)
                {
                    ...
                    Price = Price + json_Danju[xlsxDoc.read(1, i).toString()].toDouble() * xlsxDoc.read(2, i + 1).toDouble();
                }
            }
            i = i + 2;
        }
	...
	/*! 写入合计数据 */
	int Numeric = ui->spinBox->value();                      /*!< 设置小数点位数 */
    QString Str = QString("总数量：%1 ，总价格：%2").arg(QString::number(Number, 'f', Numeric)).arg(QString::number(Price, 'f', Numeric));
    xlsxDoc.write(xlsxDoc.dimension().lastRow() + 1, 1, Str);/*!< 写入excel */
	ui->textBrowser->append(Str);                            /*!< 写入日志 */

    /*! 保存文档 */
    xlsxDoc.saveAs("data.xlsx");
	ui->textBrowser->append(QString("解析文件并保存成功！"));
}
